---
name: tool-builder
description: 交互式创建和管理 Kila 当前统一工具系统里的自定义 HTTP 工具。当用户想要创建新的 API 工具、配置工具、添加自定义工具、管理自定义工具、或说"帮我创建一个 XX 工具"时使用此 Skill。也适用于调试、修复或删除已有的自定义工具。
---

# Tool Builder

通过交互式对话帮助用户创建可在 Kila 当前 session 输入区工具系统中使用的自定义 HTTP API 工具。
...
根据收集的信息构建工具配置 JSON。配置文件位于 `~/.kila/chat-tools.json`。
...
操作步骤：
1. 读取 `~/.kila/chat-tools.json`（如不存在则创建）
...
告知用户：
- "工具已创建并启用，请回到当前会话测试"
- "在当前输入框的工具选择器中应该能看到新工具"
- "试着问一个需要用到这个工具的问题"
- "如果有问题，直接在当前会话里告诉我，我帮你调试"
