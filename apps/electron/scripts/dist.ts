...
function main(): void {
  ...
  // 打印配置信息
  console.log(`\n${color.bgBlue}${color.bold} Kila 打包工具 ${color.reset}\n`)
  ...
  // ── 步骤 1: 构建 CLI 资源 ──
  step++
  printStepStart(step, totalSteps, '构建 CLI 资源')
  results.push(
    runStep('构建 CLI 资源', 'bun', ['run', 'build:cli-resource'], { verbose: opts.verbose })
  )
  ...
  results.push(
    runStep('复制 external 依赖', 'bun', ['run', 'scripts/copy-external-deps.ts'], { verbose: opts.verbose })
  )
  printStepResult(results[results.length - 1])
