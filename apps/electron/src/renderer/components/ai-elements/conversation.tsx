/**
 * AI Elements - session容器原语
 *
 * 基于 use-stick-to-bottom 实现自动滚动到底部的session容器。
 * 移植自 kila-frontend 的 ai-elements/conversation.tsx。
 *
 * 包含：
...
export function ConversationContent({ className, ...props }: ConversationContentProps): React.ReactElement {
  return (
    <StickToBottom.Content
      className={cn('flex flex-col gap-1 px-0 py-4', className)}
      {...props}
    />
  )
}
