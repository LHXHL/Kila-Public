...
export function FilePathChip({ filePath, basePath, className }: FilePathChipProps): React.ReactElement {
  ...
  const { openHtmlFileInSessionBrowser } = useSessionWebPreview()
  ...
  const handleClick = React.useCallback(() => {
    const action = isHtmlFilePath(fullPath)
      ? openHtmlFileInSessionBrowser(fullPath)
      : window.electronAPI.previewFile(fullPath)

    Promise.resolve(action).catch((error: unknown) => {
      console.error('[FilePathChip] 预览文件失败:', error)
    })
  }, [fullPath, openHtmlFileInSessionBrowser])
