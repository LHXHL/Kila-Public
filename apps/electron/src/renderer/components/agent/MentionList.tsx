...
function MentionListInner<T>(
  ...
  if (items.length === 0) {
    return (
      <div className="w-[320px] rounded-xl border bg-popover p-2 text-[11px] text-muted-foreground">
        {emptyText}
      </div>
    )
  }
  ...
  return (
    <div
      ref={containerRef}
      className="rounded-xl border bg-popover overflow-y-auto max-h-[240px] w-[320px]"
    >
      {items.map((item, index) => (
        <button
          ...
          type="button"
          className={cn(
            'flex w-full items-start gap-2 px-2.5 py-2 text-left text-xs transition-colors hover:bg-accent',
