    ...
    <div className={cn('mb-2', className)}>
      <div className="flex items-start gap-3 rounded-xl border border-primary/15 bg-primary/5 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-sm font-medium text-foreground">已从 widget 生成追问草稿</div>
          <div className="mt-1 text-sm text-muted-foreground">
            {getProposalPreview(proposal)}
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          <Button type="button" size="sm" onClick={onSend}>发送</Button>
          <Button type="button" size="sm" variant="outline" onClick={onEdit}>编辑</Button>
