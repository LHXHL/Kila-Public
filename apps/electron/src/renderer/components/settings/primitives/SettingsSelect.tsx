...
export function SettingsSelect({
  ...
  return (
    <div className="px-4 py-3 space-y-2">
      <div>
        <div id={`${descriptionId}-label`} className={LABEL_CLASS}>{label}</div>
        {description && (
          <div id={descriptionId} className={cn(DESCRIPTION_CLASS, 'mt-0.5')}>{description}</div>
        )}
      </div>
      <Select value={value} onValueChange={onValueChange} disabled={disabled}>
        <SelectTrigger className="w-full" aria-labelledby={`${descriptionId}-label`} aria-describedby={description ? descriptionId : undefined}>
