...
import type { BunRuntimeStatus, PlatformArch } from '@kila/shared'
...
export async function detectBunRuntime(): Promise<BunRuntimeStatus> {
  ...
  if (app.isPackaged) {
    ...
    if (bundledPath) {
      ...
      if (version) {
        log.info(`[Bun 检测] 找到打包的 Bun: ${bundledPath} (${version})`)
        return {
          available: true,
          path: bundledPath,
          version,
        ...
        log.warn(`[Bun 检测] 打包的 Bun 无法执行: ${bundledPath}`)
      }
    } else {
      log.warn('[Bun 检测] 打包环境中未找到 Bun 二进制文件')
    }
  ...
  if (systemPath) {
    ...
    if (version) {
      log.info(`[Bun 检测] 找到系统 Bun: ${systemPath} (${version})`)
      return {
        available: true,
        path: systemPath,
        version,
  ...
  if (vendorPath) {
    ...
    if (version) {
      log.info(`[Bun 检测] 找到 vendor Bun: ${vendorPath} (${version})`)
      return {
        available: true,
        path: vendorPath,
        version,
  ...
  // 未找到任何 Bun
  log.warn('[Bun 检测] 未找到可用的 Bun 运行时')
  return {
    available: false,
    path: null,
    version: null,
