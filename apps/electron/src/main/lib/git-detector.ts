...
import type { GitRuntimeStatus, GitRepoStatus } from '@kila/shared'
...
function findGitPath(): string | null {
  ...
  // Windows 上从注册表探测 Git 安装路径，再检查常见位置
  if (process.platform === 'win32') {
    // 优先从注册表获取安装路径
    const { getGitForWindowsInstallPath } = require('./shell-env') as typeof import('./shell-env')
    const regPath = getGitForWindowsInstallPath()
    if (regPath) {
      const candidates = [
        `${regPath}\\cmd\\git.exe`,
        `${regPath}\\bin\\git.exe`,
...
export async function detectGitRuntime(): Promise<GitRuntimeStatus> {
  log.info('[Git 检测] 开始检测 Git 运行时...')

  const gitPath = findGitPath()

  if (!gitPath) {
    log.warn('[Git 检测] 未找到 Git')
    return {
      available: false,
      version: null,
      path: null,
  ...
  if (!version) {
    log.warn(`[Git 检测] Git 无法执行: ${gitPath}`)
    return {
      available: false,
      version: null,
      path: gitPath,
  ...
  log.info(`[Git 检测] 找到 Git: ${gitPath} (${version})`)
  return {
    available: true,
    version,
    path: gitPath,
