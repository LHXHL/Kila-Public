 ...
 * 3. Bun 运行时检测
 * 4. Git 运行时检测
 * 5. Shell 环境检测（Windows - 内置 BusyBox / WSL）
 */

import type { RuntimeStatus, RuntimeInitOptions, ShellEnvironmentStatus } from '@kila/shared'
 ...
 *
 * 按顺序执行：
 * 1. loadShellEnv() - 加载 Shell 环境（macOS/Windows 打包环境）
...
export async function initializeRuntime(options: RuntimeInitOptions = {}): Promise<RuntimeStatus> {
  ...
  if (!options.skipEnvLoad) {
    try {
      ...
      log.error('[运行时初始化] Shell 环境加载失败:', error)
      envLoaded = false
    }
  }
  ...
  if (process.platform === 'win32' && !options.skipShellDetection) {
    try {
      ...
      log.error('[运行时初始化] Shell 环境检测失败:', error)
    }
  }
  ...
  log.info(`[运行时初始化] 初始化完成 (耗时 ${duration}ms)`)
  log.info('[运行时初始化] 状态:', {
