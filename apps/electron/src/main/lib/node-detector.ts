...
import type { NodeRuntimeStatus } from '@kila/shared'
...
const log = createLogger('Node Detector')
...
function findNodePath(): string | null {
  ...
  // Windows 上从注册表探测 Node.js 安装路径，再检查常见位置
  if (process.platform === 'win32') {
    // 优先从注册表获取安装路径
    const { getNodeInstallPathFromRegistry } = require('./shell-env') as typeof import('./shell-env')
    const regPath = getNodeInstallPathFromRegistry()
    if (regPath) {
      const nodeExe = `${regPath}\\node.exe`
...
export async function detectNodeRuntime(): Promise<NodeRuntimeStatus> {
  log.info('[Node.js 检测] 开始检测 Node.js 运行时...')

  const nodePath = findNodePath()

  if (!nodePath) {
    log.warn('[Node.js 检测] 未找到 Node.js')
    return {
      available: false,
      version: null,
      path: null,
  ...
  if (!version) {
    log.warn(`[Node.js 检测] Node.js 无法执行: ${nodePath}`)
    return {
      available: false,
      version: null,
      path: nodePath,
  ...
  log.info(`[Node.js 检测] 找到 Node.js: ${nodePath} (${version})`)
  return {
    available: true,
    version,
    path: nodePath,
