...
import type { ToolCall, ToolResult } from '@kila/core'
import type { AgentToolMeta, AgentToolHttpConfig } from '@kila/shared'
...
const log = createLogger('HTTP 工具')
...
export async function executeHttpTool(toolCall: ToolCall, meta: AgentToolMeta): Promise<ToolResult> {
  ...
  try {
    ...
    log.error(`[HTTP 工具] ${meta.id} 执行失败:`, error)
    return {
      toolCallId: toolCall.id,
