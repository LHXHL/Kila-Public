...
export async function extractTextFromFile(filePath: string): Promise<string> {
  ...
  assertSafeDocumentInput(filePath, ext)

  // PDF 文件
  if (ext === '.pdf') {
    return limitExtractedText(await withParseTimeout(extractPdf(filePath), filePath))
  }

  // 旧版 Word .doc 文件
  if (ext === '.doc') {
    return limitExtractedText(await withParseTimeout(extractDoc(filePath), filePath))
  }

  // Office 和 OpenDocument 格式
  if (OFFICE_EXTENSIONS.has(ext)) {
    return limitExtractedText(await withParseTimeout(extractOffice(filePath), filePath))
  }

  // 纯文本格式
  if (TEXT_EXTENSIONS.has(ext)) {
    return limitExtractedText(readFileSync(filePath, 'utf-8'))
  }

  throw new Error(`不支持的文档格式: ${ext || 'unknown'}`)
}

function assertSafeDocumentInput(filePath: string, ext: string): void {
  ...
  if (!stat.isFile()) {
    throw new Error('文档路径不是文件')
  }
  if (stat.size > MAX_DOCUMENT_BYTES) {
    throw new Error(`文档过大: ${stat.size} bytes`)
  }
}
...
function limitExtractedText(text: string): string {
  ...
  log.warn(`[文档解析] 提取文本超过 ${MAX_EXTRACTED_CHARS} 字符，已截断`)
  return text.slice(0, MAX_EXTRACTED_CHARS)
}

async function withParseTimeout<T>(work: Promise<T>, filePath: string): Promise<T> {
  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timeout = setTimeout(() => {
          reject(new Error(`文档解析超时: ${filePath}`))
        }, PARSE_TIMEOUT_MS)
      }),
    ])
  } finally {
...
async function extractPdf(filePath: string): Promise<string> {
  ...
  log.info(`[文档解析] PDF 提取完成: ${result.numpages} 页, ${result.text.length} 字符`)
  return result.text
}
...
async function extractDoc(filePath: string): Promise<string> {
  ...
  log.info(`[文档解析] DOC 提取完成: ${text.length} 字符`)
  return text
}
...
async function extractOffice(filePath: string): Promise<string> {
  ...
  log.info(`[文档解析] Office 提取完成: ${text.length} 字符`)
  return text
}
