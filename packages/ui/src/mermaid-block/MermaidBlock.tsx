 ...
 *
 * 优先使用 beautiful-mermaid 渲染，遇到不支持的图型时
 * 自动回退到官方 mermaid 渲染器。
 *
 * 核心策略 —— "源码优先，SVG 覆盖淡入"：
 *
...
/** 检查 SVG 是否可用（非空、含 <svg、无 NaN、无 mermaid 错误标记） */
...
async function renderWithOfficialMermaid(code: string): Promise<string> {
  ...
  const id = `kila-mermaid-${Date.now()}-${mermaidRenderId++}`
  const { svg } = await mermaid.render(id, code)
  if (!isUsableSvg(svg)) throw new Error('Mermaid 输出了无效 SVG')
  return svg
}

/** 渲染 Mermaid：优先 beautiful-mermaid，失败回退官方 mermaid */
async function renderMermaidSvg(code: string): Promise<string> {
  // 先尝试 beautiful-mermaid（覆盖常见图型）
  try {
    const svg = await renderMermaid(code, getThemeOptions())
...
export function MermaidBlock({ code }: MermaidBlockProps): React.ReactElement {
  ...
  const renderCurrentCode = React.useCallback(async (generation: number): Promise<void> => {
    try {
      ...
      // 渲染失败 → 保持源码展示
      if (generationRef.current === generation) setSvgHtml(null)
    }
  }, [])
  ...
  return (
    <div className="mermaid-block-wrapper group/mermaid my-2 overflow-hidden rounded-lg border border-border/60">
      {/* 头部栏 */}
      <div className="flex h-[34px] items-center justify-between bg-muted/80 px-2 py-1 text-xs text-muted-foreground">
      ...
      */}
      <div className="relative overflow-hidden">
        ...
        <pre
          className="m-0 overflow-x-auto bg-[hsl(var(--code-surface))] p-4 text-[13px] leading-[1.6] text-foreground/80"
          style={{
