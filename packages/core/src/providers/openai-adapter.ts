...
import { resolveThinkingConfig, resolveThinkingLevel } from '@kila/shared'
...
export class OpenAIAdapter implements ProviderAdapter {
  ...
  buildStreamRequest(input: StreamRequestInput): ProviderRequest {
    ...
    // 推理控制：gpt-5/o3/o4 等支持的 reasoning.effort
    ...
    if (thinkingConfig.enabled) {
      ...
      if (thinkingConfig.effort) reasoning.effort = thinkingConfig.effort
      if (thinkingConfig.budget) reasoning.max_completion_tokens = thinkingConfig.budget + 16384
      if (Object.keys(reasoning).length > 0) {
        bodyObj.reasoning = reasoning
      }
    }
