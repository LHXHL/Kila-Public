...
import { getThinkingBudgetTokens, resolveThinkingLevel, resolveThinkingConfig } from '@kila/shared'
...
export class AnthropicAdapter implements ProviderAdapter {
  ...
  buildStreamRequest(input: StreamRequestInput): ProviderRequest {
    ...
    const thinkingConfig = resolveThinkingConfig(thinkingLevel, input.reasoningPortrait)

    // 启用思考时需要更大的 max_tokens（budget_tokens 必须 < max_tokens）
    const thinkingBudget = thinkingConfig.budget
    const maxTokens = thinkingConfig.enabled
      ? (thinkingBudget ? thinkingBudget + 16384 : 32768)
      : 8192
    ...
    // 启用 extended thinking：根据 mode 选择 API 字段
    // - mode='budget'  → thinking.type='enabled', budget_tokens
    // - mode='effort'  → thinking.type='adaptive', effort（Claude 4.6+/Opus 4.7+）
    // - mode='mixed'   → thinking.type='enabled', budget_tokens + effort（Claude 4.5/4.6 Opus）
    if (thinkingConfig.enabled) {
      const thinking: Record<string, unknown> = {}
      const mode = thinkingConfig.mode ?? 'budget'

      if (mode === 'effort') {
        thinking.type = 'adaptive'
        if (thinkingConfig.effort) thinking.effort = thinkingConfig.effort
      } else if (mode === 'mixed') {
        thinking.type = 'enabled'
        if (thinkingBudget) thinking.budget_tokens = thinkingBudget
        if (thinkingConfig.effort) thinking.effort = thinkingConfig.effort
      } else {
        // budget（默认）
        thinking.type = 'enabled'
      ...
      // visibility 控制 thinking.display（仅 adaptive/summary 两种暴露方式）
      if (thinkingConfig.visibility === 'summary' || thinkingConfig.visibility === 'full') {
  ...
  parseSSELine(jsonLine: string): StreamEvent[] {
    try {
      ...
      return [{ type: 'parse_error', raw: jsonLine, error: e }]
    }
  }
