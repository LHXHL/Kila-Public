...
import type { ChatMessage, FileAttachment, ProviderType, ThinkingLevel } from '@kila/shared'
import type { ExtraCapabilitiesReasoning } from '@kila/shared'
...
/** SSE 解析错误事件 */
export interface StreamParseErrorEvent {
  type: 'parse_error'
  raw: string
  error: unknown
}
...
export interface StreamRequestInput {
  ...
  /** 统一思考等级（各适配器根据供应商 API 自行转换） */
  thinkingLevel?: ThinkingLevel
  /** 是否启用思考模式（旧字段，兼容读取） */
  thinkingEnabled?: boolean
  /**
   * 来自 Provider DB 的 reasoning 能力画像（reasoning effort/budget/level/visibility 等）。
   * 由调用方查 Provider DB 后注入；未设置时各 adapter 走老的 budget_tokens 逻辑。
   */
  reasoningPortrait?: ExtraCapabilitiesReasoning
