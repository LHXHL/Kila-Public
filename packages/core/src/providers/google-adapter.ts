...
import { getThinkingBudgetTokens, resolveThinkingConfig, resolveThinkingLevel } from '@kila/shared'
...
export class GoogleAdapter implements ProviderAdapter {
  ...
  buildStreamRequest(input: StreamRequestInput): ProviderRequest {
    ...
    // 思考模式配置：
    // - Gemini 2.5：thinkingBudget（token 预算）+ includeThoughts
    // - Gemini 3：thinkingLevel='low|high' 离散档位
    if (thinkingConfig.enabled) {
      const thinkingConfigObj: Record<string, unknown> = {
        includeThoughts: thinkingConfig.summaries !== false,
      }
      if (thinkingConfig.mode === 'level' && thinkingConfig.level) {
        thinkingConfigObj.thinkingLevel = thinkingConfig.level
      } else if (thinkingConfig.budget !== undefined) {
        thinkingConfigObj.thinkingBudget = thinkingConfig.budget
      }
      generationConfig.thinkingConfig = thinkingConfigObj
    }
    ...
    return {
      url: `${url}/v1beta/models/${input.modelId}:streamGenerateContent?alt=sse`,
      headers: {
