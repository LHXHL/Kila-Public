 ...
 *
 * 1M 上下文已随各家模型转正为默认能力，无需下发 beta header。Kila 统一按
 * 模型能力推断窗口：前端展示、上下文估算和运行时注册必须共用同一份判定，
 * 避免出现「UI 显示 1M 但实际只 200K」的不一致。
 *
 * 判定顺序：Codex 对齐 GPT-5.x 窗口 → 1M 规则表 → 默认 200K。
 * 纯函数、不依赖 Pi SDK，可在 browser-safe 的 shared 层直接使用。
 */
 ...
 * 本函数提供一个按模型家族的 fallback，保证进度指示器永远有分母可用。
 */
