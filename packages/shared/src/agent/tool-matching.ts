...
import { isAgentToolName, isSubagentToolName, normalizeAgentToolName } from './tool-name'
...
export function extractToolStarts(
  ...
  for (const block of contentBlocks) {
    ...
    const canonicalToolName = normalizeAgentToolName(toolBlock.name)
    toolIndex.register(toolBlock.id, canonicalToolName, toolBlock.input)
    ...
    } else if (!isSubagentToolName(toolBlock.name) && activeParentTools && activeParentTools.size === 1) {
...
function extractIntent(toolBlock: ToolUseBlock): string | undefined {
  ...
  if (!intent && isAgentToolName(toolBlock.name, 'Bash')) {
 ...
 *
 * 此函数在 JSON 序列化之前处理，避免 [KILA_IMAGE_ATTACHMENT:...] 标记被转义、
...
function extractFromMcpContent(content: unknown): { text: string; images: AgentToolResultImage[] } {
  ...
  for (const block of blocks) {
    ...
    // 图片信息通过 text 块中的 [KILA_IMAGE_ATTACHMENT:...] 标记携带
  }
...
export function parseToolResultImages(raw: string): ParsedToolResult {
  const { cleanedText, images } = extractKilaImageAttachments(raw)
...
function detectBackgroundEvents(
  ...
  // 后台 Task/Agent 检测
  if (isSubagentToolName(entry.name) && !isError && resultStr && entry.input.run_in_background === true) {
  ...
  // 后台 Shell 检测
  if (isAgentToolName(entry.name, 'Bash') && !isError && resultStr) {
  ...
  // Shell 终止检测
  if (isAgentToolName(entry.name, 'KillShell')) {
